...
_quiz_creation_attempts: dict[str, deque[float]] = defaultdict(deque)
_quiz_creation_lock = threading.Lock()
...
def _check_quiz_creation_rate() -> None:
    now = time.monotonic()
    ip = request.remote_addr or "unknown"
    with _quiz_creation_lock:
        attempts = _quiz_creation_attempts[ip]
        while attempts and now - attempts[0] >= QUIZ_CREATION_WINDOW_SECONDS:
            attempts.popleft()
        if len(attempts) >= QUIZ_CREATION_LIMIT:
...
@education_blueprint.get("/education/challenges/<challenge_id>/segmentation.nii.gz")
def get_challenge_segmentation(challenge_id: str):
    store = get_education_store()
    store.challenge(challenge_id)
    return send_file(
        io.BytesIO(store.reveal_segmentation()),
        mimetype="application/gzip",
        download_name=f"{challenge_id}-lesion.nii.gz",
    )
