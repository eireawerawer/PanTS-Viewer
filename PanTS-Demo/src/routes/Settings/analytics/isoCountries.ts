...
//
// The bridge between the two halves of the map. world-atlas identifies each
// country feature by its NUMERIC code ("276"), because that is what Natural
// Earth carries; GeoLite2 reports the ALPHA-2 code ("DE"), because that is what
// everything else uses. Without this table the shapes and the session counts
// can't be joined and the map draws empty.
//
// Matching on country *name* instead is the obvious shortcut and it does not
// work: Natural Earth says "United States of America" where MaxMind says
// "United States", and there are a dozen more like it.
//
// Generated from the ISO 3166-1 register (via pycountry); a fixed standard, so
// it needs revisiting only when a country is added or dissolved.
...
/** The alpha-2 code for a world-atlas feature id, or null for a feature the
 *  register doesn't cover (Natural Earth carries a few disputed territories
 *  with no ISO code of their own). */
export const alpha2ForFeatureId = (id: string | number | undefined): string | null => {
	if (id === undefined || id === null) return null;
	// Natural Earth's ids lose their leading zero when they arrive as numbers.
